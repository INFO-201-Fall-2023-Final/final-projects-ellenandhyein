...
df_suicide <- read.csv("Suicide Rate.csv")
...
colnames(df_suicide)[colnames(df_suicide) == "GDP.per.capita"] <- "GDP.per.capita.suicide"

df <- merge(x= eighten_df, y=twenty_df, by.x= c("Country.or.region"), by.y=c("Country.name"))

df <- merge(x= df, y=df_suicide, by.x= c("Country.or.region"), by.y= c("Country")) 
